...
const handleTextMessage = async (event: line.MessageEvent & { message: line.TextMessage }) => {
    ...
    if (text === 'テキストで記録') {
        ...
        return lineClient.replyMessage(event.replyToken, [{ type: 'text', text: '食事の内容を教えてください (例: ラーメンと餃子)' }]);
    }
    if (text === '過去の食事から記録') {
    ...
    return lineClient.replyMessage(event.replyToken, [{ type: 'text', text: result.response.text() }]);
};
...
const analyzeMealAndReply = async (userId: string, type: 'text' | 'image', data: string | Buffer, meal_type: string, replyToken: string) => {
    let mealText = '';
    if (type === 'image') {
        const [result] = await visionClient.labelDetection({ image: { content: data } });
    ...
    const result = await geminiModel.generateContent(prompt);
    return lineClient.replyMessage(replyToken, [{ type: 'text', text: result.response.text() }]);
};