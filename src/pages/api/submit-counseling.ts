...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const userData = { age: Number(age), height: Number(height), initialWeight: Number(weight), currentWeight: Number(weight), targetWeight: Number(targetWeight), goal, createdAt: new Date() };
        ...
        await lineClient.pushMessage({ to: userId, messages: [{ type: 'text', text: result.response.text() }] });
